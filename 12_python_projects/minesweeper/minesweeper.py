...
class Board:
    ...
    def __init__(self, dim_size, num_bombs):
        ...
        # We'll save (row, col) typles into this set 
    ...
    def assign_values_to_board(self):
        ...
        # represents how many neighboring bombs there are. We can precompute these and it'll save us some effort chechking what's around the board later on.
    ...
    def get_num_neighboring_bombs(self, row, col):
        ...
        for r in range(max(0, row-1), min(self.dim_size-1, (row+1))+1):
            for c in range(max(0, col-1), min(self.dim_size - 1,(col+1))+1):
                if r == row and c == col:
                    # Or original location, don't check
                    continue
                if self.board[r][c] == '*':
    ...
    def dig(self, row, col):
        ...
        # Dig at location with neighboring boms -> finish dig
        # Dig at location with no neightboring bombs recursively dig neighbors!
        ...
        for r in range(max(0, row-1), min(self.dim_size-1, (row+1))+1):
            for c in range(max(0, col-1), min(self.dim_size - 1,(col+1))+1):
                if(r, c) in self.dug:
                    continue # Don't did where you've already dug
                self.dig(r, c)
...
def play(dim_size=10, num_bombs=10):
    ...
    # Step 3b: If location is not a bomb, dig recursively until each each square is at least next to a bomb
    ...
    # 2 ways to end look, let's chech which one
    if safe:
        print("Gratz")
    else:
        print("Sry")
